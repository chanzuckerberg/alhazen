...
@app.cell
def __(c_ids, get_prunelist, mo, rag_data, selected_dois, set_prunelist):
    if len(c_ids) > 0:
        l = get_prunelist()
...
@app.cell
def __(end_slider, mo, start_slider):
...
@app.cell
def __(embed_df, end_slider, start_slider):
...
@app.cell
def __(alt, c_ids, embedding_df, mo):
    if len(c_ids) > 0:
...
@app.cell
def __(c_ids, ldb, map_type_dropdown, text):
    if len(c_ids)>0:
...
@app.cell
def __(alt, c_ids, mo, papers_df, pd, resample_dropdown):
...
@app.cell
def __(c_ids, chart, embedding_df, ldb, mo):
    if len(c_ids)>0:
